...
- Optional Google Antigravity OAuth credentials, created by an explicit browser sign-in and stored in macOS Keychain. QuotaDot does not read another application's Google credentials.
...
- Optional Antigravity authorization opens `accounts.google.com` in the browser. Code exchange and refresh tokens go only to `oauth2.googleapis.com/token`; access tokens go to `daily-cloudcode-pa.googleapis.com` for project and quota queries. Requests reject redirects. A temporary HTTP listener bound to `127.0.0.1` receives the authorization callback, validates state and uses PKCE; it closes on completion, cancellation, or after five minutes. No model inference requests are sent.
...
Antigravity access and refresh tokens are stored under `com.cmsjcm.QuotaDot.antigravity-oauth` with `AfterFirstUnlockThisDeviceOnly` accessibility. Quota data stays in memory. Disconnect deletes the local credentials; it does not revoke Google authorization server-side. Google authorization can also be revoked in your Google Account security settings.
